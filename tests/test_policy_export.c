...
static void build_full_snapshot(struct policy_snapshot *snap)
{
	...
	snprintf(snap->timestamp, sizeof(snap->timestamp),
	...
	/* Agent hash: all 0xBB */
	snprintf(snap->agent_path, sizeof(snap->agent_path),
		 "/usr/sbin/lota-agent");
...
static void test_emit_verifier_fields(void)
{
	...
	for (int i = 0; i < n; i++) {
		if (!contains(yaml, required_keys[i])) {
			char msg[128];
			snprintf(msg, sizeof(msg), "missing required key: %s",
				 required_keys[i]);
			FAIL(msg);
			return;
		}
...
static void test_emit_written_count(void)
{
	...
	if (written != strlen(buf)) {
		char msg[128];
		snprintf(msg, sizeof(msg), "written=%zu, strlen=%zu", written,
			 strlen(buf));
		FAIL(msg);
		return;
	}
