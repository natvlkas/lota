...
static void restore_steam_env(void)
{
	...
	restore_env("PRESSURE_VESSEL_RUNTIME_BASE",
		    saved.pressure_vessel_runtime_base);
	restore_env("PRESSURE_VESSEL_INSTANCE_ID",
		    saved.pressure_vessel_instance_id);
...
static void test_detect_soldier_container(void)
{
	...
	setenv("PRESSURE_VESSEL_RUNTIME", "/usr/lib/SteamLinuxRuntime_soldier",
	       1);
...
static void test_detect_compat_data_path(void)
{
	...
	if (strcmp(info.steam_compat_path,
		   "/home/user/.steam/compatdata/730") != 0) {
