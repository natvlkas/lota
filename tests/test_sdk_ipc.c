...
int main(void)
{
	...
	if (ret == 0) {
		char flags_str[256];
		lota_flags_to_string(status.flags, flags_str,
				     sizeof(flags_str));
	...
	test_result("lota_is_attested()", ret == 1,
		    "should return 1 for attested");
	...
	if (ret == 0) {
		...
		test_result("Client nonce echoed", nonce_match,
			    "nonce mismatch");

		/* token validity check */
		int valid_times = (token.valid_until > 0);
		test_result("Valid timestamps", valid_times,
			    "invalid timestamps");

		/* for --test-signed mode, signature should be present */
		if (token.signature_len > 0) {
			printf("  Token is TPM-SIGNED!\n");
			test_result("TPM signature present",
				    token.signature_len == 256,
				    "expected 256-byte RSA signature");
			test_result("Attest data present",
	...
	if (ret == 0) {
		...
		test_result("Token timestamp fresh", fresh,
	...
	client = lota_connect();
	test_result("Reconnect after disconnect", client != NULL,
		    "reconnect failed");

	if (client) {
		ret = lota_get_status(client, &status);
		test_result("Status after reconnect", ret == 0,
			    "status failed");
		lota_disconnect(client);
	}
	...
	if (client) {
		...
		if (ret == 0) {
			...
			test_result("lota_unsubscribe()", ret == 0,
				    lota_strerror(ret));
		}
		lota_disconnect(client);
	} else {
