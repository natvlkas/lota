			 ...
			 size_t payload_size, size_t *payload_len)
{
	...
	for (;;) {
		...
		if (resp->result == LOTA_IPC_NOTIFY && resp->payload_len > 0) {
			...
			ret = recv_exact_timeout(client->fd, nbuf, nlen,
						 client->timeout_ms);
			if (ret < 0)
				return ret;
		   ...
		   struct lota_token *token)
{
	...
	memcpy(token->runtime_protect_digest,
	...
	if (ipc_token_hdr.pid_list_size > 0) {
		token->protected_pids = calloc(token->protect_pid_count,
					       sizeof(*token->protected_pids));
			 ...
			 size_t buflen, size_t *written)
{
	...
	memcpy(wire.policy_digest, token->policy_digest,
	       sizeof(wire.policy_digest));
...
int lota_poll_events(struct lota_client *client, int timeout_ms)
{
	...
	for (;;) {
		...
		/*
		 * For infinite wait, return once at least one notification has
		 * been delivered and the currently queued frames were drained.
		 */
