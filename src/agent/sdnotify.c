...
static int validate_notify_socket_env(void)
{
	...
	if (strcmp(notify_socket, expected) == 0)
		return validate_filesystem_notify_socket(notify_socket,
							 geteuid());
...
bool sdnotify_under_systemd(void)
{
	...
	return getenv("NOTIFY_SOCKET") != NULL ||
	       getenv("INVOCATION_ID") != NULL;
}

int sdnotify_ready(void)
{
	return sdnotify_send_checked("READY=1");
}

int sdnotify_reloading(void)
{
	return sdnotify_send_checked("RELOADING=1");
}

int sdnotify_stopping(void)
{
	return sdnotify_send_checked("STOPPING=1");
}
...
{
	return sdnotify_send_checked("WATCHDOG=1");
}
