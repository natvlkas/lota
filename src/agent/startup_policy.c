...
static int canonicalize_trust_libs(char (*trust_libs)[PATH_MAX],
				   int trust_lib_count,
				   char (**out_libs)[PATH_MAX], int *out_count)
{
	...
	*out_count = 0;
	...
	if (!canon) {
		OPENSSL_cleanse(sorted,
				(size_t)trust_lib_count * sizeof(char *));
		free(sorted);
		return -ENOMEM;
	}
						 ...
						 uint8_t out_digest[32])
{
	...
	return compute_policy_digest(&pol, g_agent.policy_verity_digests,
				     g_agent.policy_verity_digest_count,
				     out_digest);
}
				 ...
				 int digest_count, uint8_t out_digest[32])
{
	...
	if (digest_count > 0) {
		for (int i = 0; i < digest_count; i++) {
			...
			if (EVP_DigestUpdate(mdctx, le, sizeof(le)) != 1 ||
			    EVP_DigestUpdate(mdctx, digests[i].digest,
					     (size_t)digests[i].len) != 1) {
				ret = -EIO;
				goto out;
			}
		}
	...
	if (policy->protect_pid_count > 0) {
		...
		qsort(sorted_pids, (size_t)policy->protect_pid_count,
		      sizeof(uint32_t), u32_cmp);
	...
	if (policy->trust_lib_count > 0) {
		...
		qsort(sorted_libs, (size_t)policy->trust_lib_count,
		      sizeof(char *), cstr_ptr_cmp);

		int unique = 0;
		for (int i = 0; i < policy->trust_lib_count; i++) {
			if (i == 0 ||
			    strcmp(sorted_libs[i], sorted_libs[i - 1]) != 0)
				unique++;
		}
		lota__write_le32(le, (uint32_t)unique);
		if (EVP_DigestUpdate(mdctx, le, sizeof(le)) != 1) {
		...
		for (int i = 0; i < policy->trust_lib_count; i++) {
			...
			if (EVP_DigestUpdate(mdctx, le, sizeof(le)) != 1 ||
			    (slen > 0 && EVP_DigestUpdate(mdctx, sorted_libs[i],
							  (size_t)slen) != 1)) {
				ret = -EIO;
				goto out;
			}
...
int agent_apply_startup_policy(const struct agent_startup_policy *policy)
{
	...
	if (ret < 0) {
		lota_err("Failed to enable bpf syscall lock: %s",
			 strerror(-ret));
		goto out_fail;
	}
	...
	if (policy->strict_mmap) {
		ret = bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_STRICT_MMAP, 1);
		if (ret < 0) {
			lota_err("Failed to enable strict mmap: %s",
				 strerror(-ret));
			goto out_fail;
		}
	...
	if (policy->allow_verity_count > 0) {
		...
		for (int i = 0; i < policy->allow_verity_count; i++) {
...

			{
				...
				hex_encode_upper(d->digest, d->len, hex,
						 sizeof(hex));
				lota_dbg("Allowed fs-verity: %s digest=%s",
					 policy->allow_verity[i], hex);
			}
		}
		...
		if (ret < 0) {
			lota_err("Failed to compute policy digest: %s",
				 strerror(-ret));
			goto out_fail;
		}
		have_policy_digest = 1;

		lota_info("fs-verity allowlist applied (%d entries)",
			  allowlist_applied);
		goto allowlist_done;
	} else if (policy->strict_exec || policy->strict_modules) {
		lota_err("Strict exec/modules requested but no allow_verity "
			 "entries are "
			 "configured");
		ret = -EINVAL;
		goto out_fail;
...
allowlist_done:
	...
	if (policy->strict_exec) {
		ret = bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_STRICT_EXEC, 1);
		if (ret < 0) {
			lota_err("Failed to enable strict exec: %s",
				 strerror(-ret));
			goto out_fail;
		}
	...
	if (policy->block_ptrace) {
		ret = bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_BLOCK_PTRACE, 1);
		if (ret < 0) {
			lota_err("Failed to enable ptrace blocking: %s",
				 strerror(-ret));
			goto out_fail;
		}
	...
	if (policy->strict_modules) {
		ret = bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_STRICT_MODULES, 1);
		if (ret < 0) {
			lota_err("Failed to enable strict modules: %s",
				 strerror(-ret));
			goto out_fail;
		}
	...
	if (policy->block_anon_exec) {
		ret = bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_BLOCK_ANON_EXEC, 1);
		if (ret < 0) {
			lota_err("Failed to enable anonymous exec blocking: %s",
				 strerror(-ret));
			goto out_fail;
		}
	...
	for (int i = 0; i < policy->trust_lib_count; i++) {
		ret = bpf_loader_trust_lib(&g_agent.bpf_ctx,
					   policy->trust_libs[i]);
		if (ret < 0) {
	...
	/* persist canonical policy snapshot for runtime recompute */
	{
		...
		ret = canonicalize_trust_libs(policy->trust_libs,
					      policy->trust_lib_count,
...
out_fail:
	...
	if (cfg_lock_bpf_applied)
		(void)bpf_loader_set_config(&g_agent.bpf_ctx, LOTA_CFG_LOCK_BPF,
					    0);
	if (cfg_block_anon_exec_applied)
		(void)bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_BLOCK_ANON_EXEC, 0);
	if (cfg_strict_modules_applied)
		(void)bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_STRICT_MODULES, 0);
	if (cfg_block_ptrace_applied)
		(void)bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_BLOCK_PTRACE, 0);
	if (cfg_strict_exec_applied)
		(void)bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_STRICT_EXEC, 0);
	if (cfg_strict_mmap_applied)
		(void)bpf_loader_set_config(&g_agent.bpf_ctx,
					    LOTA_CFG_STRICT_MMAP, 0);
...
validate_protected_pid_capacity(const struct agent_startup_policy *policy)
{
	...
	ret = canonicalize_u32_set(policy->protect_pids,
				   policy->protect_pid_count, &canon,
				   &canon_count);
	if (ret < 0)
		return ret;
