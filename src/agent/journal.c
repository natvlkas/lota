...
void journal_init(const char *ident)
{
	...
	g_use_journal = (getenv("JOURNAL_STREAM") != NULL ||
		   ...
		   const char *fmt, ...)
{
	...
	if (g_use_journal) {
		sd_journal_send("MESSAGE=%s", buf, "PRIORITY=%d", priority,
				"SYSLOG_IDENTIFIER=%s", g_ident, "CODE_FILE=%s",
				file, "CODE_LINE=%d", line, "CODE_FUNC=%s",
				func, NULL);
	} else {
		/*
